    ...
    for i in tqdm(range(total_frames)):
        ...
        for track in tracked_objects:
            ...
            color = cv2.applyColorMap(
                np.array(((track_id * 30) % 255), dtype="uint8"), cv2.COLORMAP_HSV
            )[0][0]
...
def connect_tracklets(video_path: str, out_dir: str, max_time: int, overwrite: bool):
    ...
    PROB_NO_DETECTION = 1e-6
    T_DIST_DF = 12
    MIN_PROB_THRESH = 1e-15
...
def find_shortest_paths(data, diff_coeff, prob_no_detection, df, min_prob):
    ...
    dist_graph = calculate_dist_graph(data, diff_coeff, prob_no_detection, df, min_prob)
...
def calculate_dist_graph(data, diff_coeff, prob_no_detection, df, min_prob):
    dist_px = calculate_dist_px(data)
    dist_gap = calculate_dist_gap(data)
    dist_confidence = calculate_dist_confidence(data)
    dist_count = calculate_dist_count(data)
    dist_graph = (
        neg_log_p_dist2D(dist_px, diff_coeff, df, min_prob)
        - np.log(dist_confidence)
        - (np.log(1 - np.power(1 - prob_no_detection, dist_count)))
        - (np.abs(dist_gap - 1) * np.log(prob_no_detection))
    )
    dist_graph[:, 0] = 0
    dist_graph[-1, :] = 0
    dist_graph[0, -1] = 0
    ...
    dist_graph[dist_gap < 1] = 0
    return dist_graph


def neg_log_p_dist2D(dist_px, diff_coeff, df, min_prob):
    std = np.sqrt(2 * 2 * diff_coeff)
    neg_log_p = -np.log(min_prob + 2 - 2 * stdtr(df, dist_px / (np.sqrt(2) * std)))
    return neg_log_p
